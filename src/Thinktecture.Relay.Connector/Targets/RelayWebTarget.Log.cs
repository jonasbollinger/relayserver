...
public partial class RelayWebTarget<TRequest, TResponse>
{
	...
	{
		[LoggerMessage(LoggerEventIds.RelayWebTargetRequestingTarget, LogLevel.Trace, "Requesting target for request {RelayRequestId} at {BaseAddress} for {Url}")]
		public static partial void RequestingTarget(ILogger logger, Guid relayRequestId, Uri? baseAddress, string url);

		[LoggerMessage(LoggerEventIds.RelayWebTargetRequestedTarget, LogLevel.Debug, "Requested target for request {RelayRequestId} returned {HttpStatusCode}")]
