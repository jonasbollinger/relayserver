...
// ReSharper restore RedundantNameQualifier
{
		...
		Uri baseAddress)
	{
		...
		{
			RelayWebTargetOptions.FollowRedirect => httpClientFactory.CreateClient(Constants.HttpClientNames
				.RelayWebTargetFollowRedirect),
...
public static class RelayConnectorBuilderExtensions
{
	...
	public static IRelayConnectorBuilder<TRequest, TResponse, TAcknowledge> AddWebTarget<TRequest, TResponse,
		TAcknowledge>(
		this IRelayConnectorBuilder<TRequest, TResponse, TAcknowledge> builder, string id, Uri baseAddress,
		TimeSpan? timeout = null)
		where TRequest : IClientRequest
		where TResponse : ITargetResponse, new()
		where TAcknowledge : IAcknowledgeRequest
		=> builder.AddTarget<TRequest, TResponse, TAcknowledge, RelayWebTarget<TRequest, TResponse>>(id, timeout,
			baseAddress);
}
...
public static class RelayTargetRegistryExtensions
{
	...
	/// <typeparam name="TRequest">The type of request.</typeparam>
	/// <typeparam name="TResponse">The type of response.</typeparam>
	public static void RegisterWebTarget<TRequest, TResponse>(
		this RelayTargetRegistry<TRequest, TResponse> relayTargetRegistry,
...
{
	/// <summary>
	/// Creates an instance of a class implementing <see cref="ITargetResponse"/> from <paramref name="request"/> and
	...
	/// implementing <see cref="ITargetResponse"/>.
	/// </returns>
	public static async Task<TResponse> CreateResponseAsync<TResponse>(this HttpResponseMessage message,
		...
		where TResponse : ITargetResponse, new()
	{
		...
		response.OriginalBodySize = hasBody ? message.Content.Headers.ContentLength : 0;
