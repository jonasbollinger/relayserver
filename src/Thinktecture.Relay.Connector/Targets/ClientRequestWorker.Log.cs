...
public partial class ClientRequestWorker<TRequest, TResponse>
{
	private static partial class Log
	{
		...
		private static readonly Action<ILogger, Guid, TRequest, Exception?> LoggerErrorProcessingRequest
			= LoggerMessage.Define<Guid, TRequest>(LogLevel.Error,
				LoggerEventIds.ClientRequestWorkerErrorProcessingRequest,
				"An error occured while processing request {RelayRequestId} {@Request}");

		public static void ErrorProcessingRequest(ILogger logger, Exception exception, Guid requestRequestId,
			TRequest request)
		{
			if (logger.IsEnabled(LogLevel.Error))
			{
				LoggerErrorProcessingRequest.Invoke(logger, requestRequestId, request, exception);
			}
		}
	}
}
